...
class UploadController extends Controller {
  ...
  public function upload(Request $request)
  {
    ...
    $request->validate([
      ...
      'file' => 'required|max:10000000|mimes:doc,docx,png,jpeg,pdf,xlsx,xls,ppt,pptx,txt,quicktime,mpeg,mp4,m4v,mov'
//        'file' => 'required|mimestype:video/avi,video/mpeg,video/quicktime'
    ]);

          $upload_dir = 'school-' . auth()->user()->school_id . '/' . date("Y") . '/' . $request->upload_type;

          $path = Storage::disk('uploads')->putFile($upload_dir, $request->file('file'));
...
//      $path =  $request->file('file')->store($upload_dir);
          ...
          if ($request->upload_type == 'notice') {
              ...
              $tb->pic_path = 'upload/' . $path;
              $tb->save();
          }

          return ($path) ? response()->json([
              'imgUrlpath' => url('upload/' . $path),
              'path' => 'upload/' . $path,
              'error' => false
          ]) : response()->json([
              'imgUrlpath' => null,
